...
float ADCOneshotSensor::sample()
{
  ...
  ESP_LOGV(TAG, "ADC%d Channel[%d] Raw Data: %d", ADC_UNIT_1 + 1, this->channel1_, adc_raw[0][0]);
  if (do_calibration1_chan0) {
      // Convert the ADC raw result into calibrated result
      ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_chan0_handle, adc_raw[0][0], &voltage[0][0]));
      ESP_LOGV(TAG, "ADC%d Channel[%d] Calibrated Voltage: %d mV", ADC_UNIT_1 + 1, this->channel1_, voltage[0][0]);
  }
...
static bool adc_calibration_init(adc_unit_t unit, adc_channel_t channel, adc_atten_t atten, adc_cali_handle_t *out_handle)
{
    ...
    if (!calibrated) {
        ESP_LOGV(TAG, "calibration scheme version is %s", "Curve Fitting");
    ...
    if (!calibrated) {
        ESP_LOGV(TAG, "calibration scheme version is %s", "Line Fitting");
    ...
    *out_handle = handle;
    if (ret == ESP_OK) {
        ESP_LOGV(TAG, "Calibration Success");
...
{
#if ADC_CALI_SCHEME_CURVE_FITTING_SUPPORTED
    ESP_LOGV(TAG, "deregister %s calibration scheme", "Curve Fitting");
    ESP_ERROR_CHECK(adc_cali_delete_scheme_curve_fitting(handle));

#elif ADC_CALI_SCHEME_LINE_FITTING_SUPPORTED
    ESP_LOGV(TAG, "deregister %s calibration scheme", "Line Fitting");
